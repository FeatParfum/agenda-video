...
export default async function WeekDetailPage({
  ...
  searchParams: Promise<{ reservado?: string; editado?: string }>;
}) {
  ...
  const { reservado, editado } = await searchParams;
    ...
    <div className="mx-auto max-w-3xl w-full px-4 sm:px-6 py-8 sm:py-12">
      ...
      {reservado && (
        <div className="mb-6 rounded-xl bg-[#e4f3e6] border border-[#bfe6c6] px-4 py-3 text-sm text-[#1f7a32]">
          Reserva enviada com sucesso! O administrador organiza o horário definitivo até sexta-feira às 12h.
        </div>
      )}
      {editado && (
        <div className="mb-6 rounded-xl bg-[#e4f3e6] border border-[#bfe6c6] px-4 py-3 text-sm text-[#1f7a32]">
        ...
        <Card className="p-5 mb-6">
          ...
          {!happened && (
            <p className="mt-3 text-xs text-[#7a716a]">
              Agendamentos e alterações para esta segunda podem ser feitos até{" "}
              <strong>sexta-feira, {deadline.toLocaleDateString("pt-BR")} às 12h</strong>.
            </p>
          )}
          {happened && (
            <p className="mt-3 text-xs text-[#7a716a]">
              ...
              para ver os horários aprovados e enviar o relatório de gravação.
            </p>
          )}
        </Card>
      )}
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        {canBook && (
          <LinkButton href={`/segunda/${data}/agendar`} variant="primary">
            Solicitar gravação
          </LinkButton>
        )}
      </div>
            ...
            <Card key={b.id} className="p-4 sm:p-5">
              ...
              {(user.role === "admin" || (b.team_member_id === user.id && !happened)) && (
                <div className="mt-2 flex flex-wrap gap-3 items-center">
                  {b.team_member_id === user.id && !happened && isBookingOpen(data) && (
                    <Link
                      href={`/segunda/${data}/editar/${b.id}`}
                      className="text-sm font-semibold text-laranja hover:underline"
                    >
                      Editar
                    </Link>
                  )}
      ...
      {user.role === "admin" && (
        <div className="mt-6">
          <LinkButton href={`/admin/segunda/${data}`} variant="secondary">
